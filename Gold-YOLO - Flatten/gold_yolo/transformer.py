...
class FlattenAttention(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        # print('-----------------------------ALL---------------------------')
        fq = self.to_flattenq(x.view(B, -1, self.dim)).view(B, self.nh_kd, H, W).reshape(
            B, self.num_heads, self.key_dim, H * W).permute(0, 1, 3, 2)
        fk = self.to_flattenk(x.view(B, -1, self.dim)).view(B, self.nh_kd, H, W).reshape(
            B, self.num_heads, self.key_dim, H * W).permute(0, 1, 3, 2)
        fv = self.to_testV(x.view(B, -1, self.dim)).view(B, self.dh, H,
                                                         W).reshape(B, self.num_heads, self.d, H * W).permute(0, 1, 3, 2)
...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        qq = self.to_q(x).reshape(B, self.num_heads,
                                  self.key_dim, H * W).permute(0, 1, 3, 2)
        kk = self.to_k(x).reshape(B, self.num_heads, self.key_dim, H * W)
        vv = self.to_v(x).reshape(B, self.num_heads,
                                  self.d, H * W).permute(0, 1, 3, 2)
...
class top_Block(nn.Module):
    ...
    def __init__(self, dim, key_dim, num_heads, mlp_ratio=4., attn_ratio=2., drop=0.,
        ...
        #                       norm_cfg=norm_cfg)
        self.attn = FlattenAttention(dim, key_dim=key_dim, num_heads=num_heads, attn_ratio=attn_ratio, activation=act_layer,
                                     norm_cfg=norm_cfg)
